...
import io.github.sagaraggarwal86.jmeter.bpm.util.TypeConverters;
...
public final class WebVitalsCollector implements MetricsCollector<WebVitalsResult> {
    ...
    public WebVitalsResult collect(CdpCommandExecutor executor, MetricsBuffer buffer) {
        ...
        double lcp = TypeConverters.toDouble(vitals.get("lcp"));
        double cls = TypeConverters.toDouble(vitals.get("cls"));
        double fcp = TypeConverters.toDouble(vitals.get("fcp"));
        double ttfb = TypeConverters.toDouble(vitals.get("ttfb"));
        ...
        // SPA stale FCP detection — FCP only fires once per full page load
        Long fcpResult;
        ...
        // SPA stale TTFB detection — TTFB only updates on full page navigation
        Long ttfbResult;
        ...
        // CLS delta — per-action layout shift, not page-session accumulated total
        ...
        return new WebVitalsResult(fcpResult, lcpResult, clsDelta, ttfbResult);
    }
    ...
    public void reset() {
        ...
        previousFcpByThread.clear();
        previousTtfbByThread.clear();
        previousClsByThread.clear();
    }
}