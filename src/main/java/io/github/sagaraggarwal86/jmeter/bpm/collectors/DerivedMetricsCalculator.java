...
public final class DerivedMetricsCalculator {
    ...
    // Performance score weights delegated to BpmConstants (single source of truth)
    ...
    public DerivedMetrics compute(WebVitalsResult vitals, NetworkResult network,
        ...
        // Extract raw values — keep nullable for score computation
        ...
        long renderTime = (lcp != null && ttfb != null) ? Math.max(0, lcpVal - ttfbVal) : 0L;
        ...
        // Stability Category from CLS value
        ...
        // Headroom = percentage of LCP budget remaining before Poor threshold
    ...
    List<String> detectImprovementAreas(int failedRequests, long ttfb, long lcp,
        ...
        if (failedRequests > 0) {
            detected.add(BpmConstants.BOTTLENECK_RELIABILITY);
        }
        ...
        if (lcp > 0) {
            ...
            if (serverRatio > properties.getBottleneckServerRatio()) {
                detected.add(BpmConstants.BOTTLENECK_SERVER);
            }
        }
        ...
        if (lcp > 0 && !slowest.isEmpty()) {
            ...
            if (resourceRatio > properties.getBottleneckResourceRatio()) {
                detected.add(BpmConstants.BOTTLENECK_RESOURCE);
            }
        }
        ...
        if (lcp > 0) {
            ...
            if (clientRatio > properties.getBottleneckClientRatio()) {
                detected.add(BpmConstants.BOTTLENECK_CLIENT);
            }
        }
        ...
        if (domNodes > 0) {
            ...
            if (layoutCount > threshold) {
                detected.add(BpmConstants.BOTTLENECK_LAYOUT);
            }
        }
