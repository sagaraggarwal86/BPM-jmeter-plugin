...
public final class CdpSessionManager {
    ...
    public void reInjectObservers(CdpCommandExecutor executor) {
        ...
        executor.executeScript(JsSnippets.REINJECT_OBSERVERS);

        // Console hook is idempotent — re-injecting resets window.__bpm_console
        executor.executeScript(JsSnippets.CONSOLE_CAPTURE_HOOK);

        // Set marker for ensureObserversInjected() navigation detection
        executor.executeScript(JsSnippets.SET_OBSERVER_MARKER);
     ...
     * {@code false} if the existing observers are still active.
     */
