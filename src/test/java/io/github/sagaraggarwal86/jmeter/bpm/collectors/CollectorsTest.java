...
class CollectorsTest {
    ...
    class WebVitalsCollectorTests {
        ...
        void collect_lcpUnchanged_returnsPartialWithNullLcp() {
            ...
            assertNull(second.fcp(), "FCP should be null for stale SPA detection");
            assertEquals(0.0, second.cls(), 0.0001, "CLS should be preserved (accumulates)");
            assertNull(second.ttfb(), "TTFB should be null for stale SPA detection");
        }
        ...
        void collect_zeroLcpFcp_returnsNull() {
            ...
            assertNotNull(result);
            assertNull(result.lcp(), "lcp=0 means no LCP event fired — must be null, not a perfect 0 ms score");
            assertNull(result.fcp(), "fcp=0 means no FCP event fired — must be null, not a perfect 0 ms score");
    ...
    class ConsoleCollectorTests {
        ...
        void collect_withSanitizer_redactsSensitiveData() {
            buffer.addConsoleMessage("error", "Authorization: Bearer eyJhbGciOiJIUzI1NiJ9.test.sig");
